...
describe('Lift', () => {
  describe('Payments', () => {
    ...
    it('VEN-15594_lift_home_should_access_Research_Email_Confirmation_page_successfully', { tags: '@smoke' }, function () {
      cy.intercept('GET', `${ Cypress.config().lift.baseUrl }/stored_payments/my_summary`).as('paymentsSummary');

      homePage.clickPaymentsSideMenu()
      homePage.paymentsResearch.wait(1000).click()
      homePage.clickPaymentsResearchMySummary()

      cy.wait('@paymentsSummary').its('response.statusCode').should('be.oneOf', [ 200, 201, 202 ]);
